...
def limpar_nome_arquivo(nome):
    ...
    sem_espaco = sem_acento.replace(' ', '_')
    
    # Remove caracteres não permitidos, mantendo letras, números, -, _
    permitido = string.ascii_letters + string.digits + "-_"
    nome_limpo = ''.join(c for c in sem_espaco if c in permitido)
...
class BorderoApp:
    ...
    def gerar_recibo(self):
        ...
        try:
            ...
            nome_produto_limpo = limpar_nome_arquivo(titulo_produto)
            nome_arquivo_limpo_docx = f"Recibo-{nome_produto_limpo}.docx"
            arquivo_saida = filedialog.asksaveasfilename(
                defaultextension=".docx",
                initialfile=nome_arquivo_limpo_docx,
                filetypes=[("Documentos Word", "*.docx")]
            )
            if arquivo_saida:
                document.save(arquivo_saida)
                time.sleep(1)  # Dá tempo para o sistema liberar o arquivo

                # Caminho do PDF temporário (com nome limpo)
                caminho_pdf_limpo = arquivo_saida.replace(".docx", ".pdf")

                # Caminho do PDF final (com nome original e acentos)
                nome_pdf_original = f"Recibo - {titulo_produto}.pdf"
                diretorio_pdf = os.path.dirname(arquivo_saida)
                caminho_pdf_original = os.path.join(diretorio_pdf, nome_pdf_original)

                try:
                    converter_docx_para_pdf(arquivo_saida, caminho_pdf_limpo)
                    os.remove(arquivo_saida)  # Remove o .docx após conversão (opcional)

                    # Renomeia o PDF para o nome original (com acentos e espaços)
                    if os.path.exists(caminho_pdf_original):
                        os.remove(caminho_pdf_original)  # Remove arquivo antigo, se existir
                    os.rename(caminho_pdf_limpo, caminho_pdf_original)

                    messagebox.showinfo("Sucesso", f"Recibo salvo como PDF em:\n{caminho_pdf_original}")
                except Exception as e:
    ...
    def gerar_lista_detalhada(self):
        ...
        try:
            ...
            # Parte alterada para salvar e renomear o PDF:
            nome_produto_limpo = limpar_nome_arquivo(titulo_produto)
            nome_arquivo_limpo_docx = f"Lista_Detalhada-{nome_produto_limpo}.docx"
            arquivo_saida = filedialog.asksaveasfilename(
                defaultextension=".docx",
                initialfile=nome_arquivo_limpo_docx,
                filetypes=[("Documentos Word", "*.docx")]
            ...
            if arquivo_saida:
                ...
                nome_pdf_original = f"Lista Detalhada - {titulo_produto}.pdf"
                diretorio_pdf = os.path.dirname(arquivo_saida)
                caminho_pdf_original = os.path.join(diretorio_pdf, nome_pdf_original)

                try:
                    converter_docx_para_pdf(arquivo_saida, caminho_pdf_limpo)
                    os.remove(arquivo_saida)  # Remove .docx após conversão

                    if os.path.exists(caminho_pdf_original):
                        os.remove(caminho_pdf_original)  # Remove PDF antigo se existir
                    os.rename(caminho_pdf_limpo, caminho_pdf_original)  # Renomeia PDF para nome original

                    messagebox.showinfo("Sucesso", f"Lista Detalhada salva como PDF em:\n{caminho_pdf_original}")
                except Exception as e:
                    messagebox.showerror("Erro", f"Erro ao converter para PDF: {e}")
